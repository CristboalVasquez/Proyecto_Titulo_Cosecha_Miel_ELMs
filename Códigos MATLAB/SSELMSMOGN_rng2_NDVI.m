...
fprintf('1. Cargando base de datos hiperdimensional (56 variables)\n');
C_data = readcell('BaseDeDatosSS2_NDVI.xlsx'); 
M = str2double(strrep(string(C_data), ',', '.'));


if any(isnan(M(1, :)))
    X_total = M(2:end, 1:56); Y_total = M(2:end, 57);
else
    X_total = M(:, 1:56);      Y_total = M(:, 57); 
end
...
% ACTUALIZADO: Escritura en la columna 57 para las muestras predichas
for idx = 1:nU
    C_final{offset + idx, 57} = max(0, min(90, pred_unlabeled(idx))); 
end

writecell(C_final, sprintf('BaseDeDatos_201_muestras_NDVI.xlsx', char(datetime('now', 'Format', 'yyyyMMdd_HHmmss'))));
...
title(sprintf('A - RMSE Mínimo: %.2f', min_rmse_global), 'FontSize', 12); 
grid on;
axis tight;
...
xlabel('Cantidad de Muestras de Testeo usadas como Entrenamiento (Xu)', 'FontSize', 11); 
ylabel('Regularización log_{10}(\lambda)', 'FontSize', 11);
title(sprintf('B - R Óptimo: %.3f', mejorR_SS), 'FontSize', 12); 
grid on;
axis tight;
