...
base_datos = 'BaseDeDatos_201_muestras_NDVI.xlsx';
C_data = readcell(base_datos);
M_raw = str2double(strrep(string(C_data), ',', '.'));

% Limpiamos los encabezados si existen (Fila 1 se vuelve NaN al convertir a número)
...
% NUEVOS ÍNDICES: 56 Variables de Entrada, 1 Salida
datos_temp    = M(:, 1:34);    
...
%% NORMALIZACIÓN GLOBAL [-1, 1] PARA APPS (Dataset 201 muestras - Con NDVI)
fprintf('\nAplicando normalización global a variables climáticas y NDVI...\n');
...
%%  CONSTRUCCIÓN DE LA TABLA Y EXPORTACIÓN AL EXCEL
fprintf('Construyendo la base de datos normalizada completa para la App...\n');

% 1. Juntar las piezas: Entradas normalizadas (Temp, Precip, NDVI) + Salida (Cosecha) en crudo
M_norm = [temp_norm, precip_norm, ndvi_norm, datos_cosecha];

% 2. Convertir la matriz de vuelta a formato 'table'
T_norm = array2table(M_norm);

% 3. Asignar nombres limpios a las columnas (1 a 56 para entradas, 57 para cosecha)
for i = 1:56
    T_norm.Properties.VariableNames{i} = ['Var_Entrada_', num2str(i)];
end
T_norm.Properties.VariableNames{57} = 'Cosecha_kg';

% 4. Mostrar resumen en consola para validación
...
fprintf('NDVI (Normalizado)  - Min: %.2f, Max: %.2f\n', min(ndvi_norm(:)), max(ndvi_norm(:)));

% 5. Guardar archivo Excel para la App con un nombre NUEVO y único
timestamp = datetime('now', 'Format', 'yyyyMMdd_HHmmss');
nombre_archivo_salida = sprintf('BaseDeDatos201_NDVI.xlsx', timestamp);
...
K = 5;
% rng(42); % (Opcional) semilla para que los folds sean consistentes
...
for j = 1:K
    ...
    % Concatenación de las 56 variables
...
RMSE_train_matrix_REGU = zeros(length(C_vec), length(NO_vec_R)); 
R_train_matrix_REGU    = zeros(length(C_vec), length(NO_vec_R)); 
R2_train_matrix_REGU   = zeros(length(C_vec), length(NO_vec_R)); 
...
RMSE_train_matrix_MELM2 = zeros(length(NO1_vec_MELM2), length(NO2_vec_MELM2)); 
R_train_matrix_MELM2    = zeros(length(NO1_vec_MELM2), length(NO2_vec_MELM2)); 
R2_train_matrix_MELM2   = zeros(length(NO1_vec_MELM2), length(NO2_vec_MELM2)); 
...
fprintf('\ HIPERPARÁMETROS ÓPTIMOS SELECCIONADOS (SEGÚN RMSE) \n');
...
fprintf('  Tiempo=%.6f s, Velocidad=%.2f muestras/s\n', ...
    Time_vec_ELM(idx_best_ELM), numSamples / Time_vec_ELM(idx_best_ELM));
...
fprintf('  Tiempo=%.6f s, Velocidad=%.2f muestras/s\n', ...
    Time_matrix_REGU(idx_c, idx_no), numSamples / Time_matrix_REGU(idx_c, idx_no));
...
hold on;
plot(NO2_vec_MELM2(idx2_melm2), NO1_vec_MELM2(idx1_melm2), 'xg', 'MarkerSize', 14, 'LineWidth', 3);
hcb1 = colorbar;
...
hold on;
plot(NO2_vec_MELM2(idx2_max_r_melm2), NO1_vec_MELM2(idx1_max_r_melm2), 'xg', 'MarkerSize', 14, 'LineWidth', 3);
hcb2 = colorbar;
...
title(sprintf('A'), 'FontSize', 11); grid on; view(45, 30);
...
timestamp = datetime('now', 'Format', 'yyyyMMdd_HHmmss');
nombre_archivo = sprintf('Comparativa_Final_N201_NDVI.xlsx', timestamp);
