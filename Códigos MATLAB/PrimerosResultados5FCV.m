...
%% 1. LECTURA DE DATOS
T_aux = readtable('regresionabejas.xlsx', 'ReadVariableNames', false);
M = zeros(size(T_aux));
...
%% 2. NORMALIZACIÓN GLOBAL [-1, 1] PARA APPS
fprintf('Aplicando normalización global a variables climáticas...\n');
...
% Reconstruir la matriz completa (Entradas normalizadas + Salida en crudo)
M_norm = [temp_norm, precip_norm, datos_cosecha];
Inputs_NNF = [temp_norm, precip_norm];
Target_NNF = [datos_cosecha];
% Convertir la matriz de vuelta a formato 'table' respetando los nombres
...
nombre_archivo_salida = 'regresionabejas_normalizado_APPs.xlsx';
...
fprintf('  Tiempo=%.6f s\n', Time_matrix_MELM3(idx1_star, idx2_star, idx3_star));
...
fprintf('  Tiempo=%.6f s\n', Time_matrix_SS(idx_L_optimo, idx_X_optimo));
...
nombre_archivo = sprintf('Comparativa_Final_%s.xlsx', timestamp);
